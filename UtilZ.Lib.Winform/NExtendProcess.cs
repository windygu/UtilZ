...
namespace UtilZ.Lib.Winform
{
    ...
    /// Process扩展方法类
    /// </summary>
    public static class NExtendProcess
    {
        /// <summary>
        /// 在窗口控件中启动应用程序
        /// </summary>
        ...
        public static NProcess Start(this Process process, System.Windows.Forms.Control containerControl)
        {
            if (process == null)
            {
                throw new ArgumentNullException(NExtendObject.GetVarName(p => process));
            }

            NProcess nprocess = new NProcess(process);
    ...
    public class NProcess
    {
        ...
        /// 私有构造函数不允许实例化
        /// </summary>
        internal NProcess(Process process)
        {
        ...
        internal void StartNormalApp(System.Windows.Forms.Control containerControl)
        {
            ...
            NProcess.SetWindowLong(this._process.MainWindowHandle, NProcess.GWL_STYLE, NProcess.WS_VISIBLE);

            // Put it into this form
            NProcess.SetParent(this._process.MainWindowHandle, containerControl.Handle);

            // Move the window to overlay it on this window
            NProcess.MoveWindow(this._process.MainWindowHandle, 0, 0, containerControl.Width, containerControl.Height, true);
