...
namespace UtilZ.Lib.BaseEx.LogExtend
{
    ...
    public class Log4Loger : BaseLoger
    {
        ...
        private void LogFrameworkOutput(LogItem item)
        {
            ...
            {
                Base.Log.LogSysInnerLog.OnRaiseLog(this, exi);
            }
        }
        ...
        private void RecordLog(IConfig config, LogItem item)
        {
            try
            {
                ...
                {
                    name = Base.Log.Model.LogConstant.DefaultLogRecorderName;
                }
                ...
                item.LogProcess();
                string content = LayoutManager.LayoutLog(item, config, true);
                switch (item.Level)
                {
                    case LogLevel.Debug:
                        log.Debug(content);
            ...
            {
                Base.Log.LogSysInnerLog.OnRaiseLog(this.GetType().FullName, exi);
            }
        }
    }
}
