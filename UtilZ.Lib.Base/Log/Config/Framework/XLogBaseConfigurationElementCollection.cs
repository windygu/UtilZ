...
namespace UtilZ.Lib.Base.NLog.Config.Framework
{
    ...
    public class XLogBaseConfigurationElementCollection<T> : System.Configuration.ConfigurationElementCollection where T : BaseLogConfig, new()
    {
        /// <summary>
        ...
        public void Add(BaseLogConfig item)
        {
            int index = base.BaseIndexOf(item);
            if (index >= 0)
            {
        ...
        public void Remove(BaseLogConfig item)
        {
            int index = base.BaseIndexOf(item);
            if (index >= 0)
            {
