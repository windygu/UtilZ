...
namespace UtilZ.Lib.Base.Extend
{
    ...
    /// 文件类扩展方法类[分区文本文件创]
    /// </summary>
    public static class NExtendFile
    {
        /// <summary>
        ...
        {
            //参数检查
            NExtendFile.CreateFilePathParaCheck(index, dateFormat, directory, fileSize);

            if (index == -1)
            {
                index = NExtendFile.UpdateFileIndex(directory, extension, createTime, dateFormat, fileName);
            }
            else
            {
