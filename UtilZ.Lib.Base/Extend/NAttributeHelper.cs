...
namespace UtilZ.Lib.Base.Extend
{
    ...
    public class NAttributeHelper
    {
            ...
            where W : Attribute
        {
            if (valueObj == null)
            {
                throw new ArgumentNullException(NExtendObject.GetVarName(p => valueObj));
            }

            if (targetObj == null)
            {
                throw new ArgumentNullException(NExtendObject.GetVarName(p => targetObj));
            }
