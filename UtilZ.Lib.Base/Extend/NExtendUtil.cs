...
namespace UtilZ.Lib.Base.Extend
{
    ...
    /// 通用扩展类
    /// </summary>
    public static class NExtendUtil
    {
        /// <summary>
        /// 触发事件
        /// </summary>
        ...
        public static void OnRaise<T>(this EventHandler<NTEventArgs<T>> eventHandler, object sender, T para)
        {
            var handler = eventHandler;
            if (handler != null)
            {
                handler(sender, new NTEventArgs<T>(para));
            }
        }
