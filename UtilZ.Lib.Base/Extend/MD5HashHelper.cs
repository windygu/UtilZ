...
namespace UtilZ.Lib.Base.Extend
{
    ...
    public static class MD5HashHelper
    {
        ...
        public static string GetHash(byte[] data, MD5HashType hashType = MD5HashType.SHA1Managed)
        {
            if (data == null)
            {
                throw new ArgumentNullException(NExtendObject.GetVarName(p => data));
            }

            using (HashAlgorithm entry = MD5HashHelper.CreateHashEntry(hashType))
            {
                //计算指定Stream 对象的哈希值
    ...
    public enum MD5HashType
    {
        ...
        /// </summary>
        [NDisplayNameAttribute(DisplayName = "HMACSHA1", Description = "使用SHA1哈希函数计算基于哈希值的消息验证代码(HMAC),同样的数据每次计算出的hash会是是不一样的")]
        HMACSHA1,

        /// <summary>
        /// 使用TripleDES计算输入数据CryptoStream的消息验证代码(MAC)
        /// </summary>
        [NDisplayNameAttribute(DisplayName = "MACTripleDES", Description = "使用TripleDES计算输入数据CryptoStream的消息验证代码(MAC")]
        MACTripleDES,

        /// <summary>
        /// 使用加密服务提供程序(CSP)提供的实现，计算输入数据的MD5哈希值
        /// </summary>
        [NDisplayNameAttribute(DisplayName = "MD5CryptoServiceProvider", Description = "使用加密服务提供程序(CSP)提供的实现,计算输入数据的MD5哈希值")]
        MD5CryptoServiceProvider,

        /// <summary>
        /// 使用托管库计算输入数据的SHA1哈希值
        /// </summary>
        [NDisplayNameAttribute(DisplayName = "SHA1Managed", Description = "使用托管库计算输入数据的SHA1哈希值")]
        SHA1Managed,

        /// <summary>
        /// 使用托管库计算输入数据的SHA256哈希值
        /// </summary>
        [NDisplayNameAttribute(DisplayName = "SHA256Managed", Description = "使用托管库计算输入数据的SHA256哈希值")]
        SHA256Managed,

        /// <summary>
        /// 使用托管库计算输入数据的SHA384哈希值
        /// </summary>
        [NDisplayNameAttribute(DisplayName = "SHA384Managed", Description = "使用托管库计算输入数据的SHA384哈希值")]
        SHA384Managed,

        /// <summary>
        /// 使用托管库计算输入数据的SHA512哈希算法
        /// </summary>
        [NDisplayNameAttribute(DisplayName = "SHA512Managed", Description = "使用托管库计算输入数据的SHA512哈希算法")]
        SHA512Managed
    }
}
