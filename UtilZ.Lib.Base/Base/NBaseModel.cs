...
namespace UtilZ.Lib.Base
{
    ...
    /// </summary>
    [Serializable]
    public class NBaseModel : INotifyPropertyChanged
    {
        ...
        /// 显示格式特性类型
        /// </summary>
        private static readonly Type _formatAttributeType = typeof(NDisplayFormatAttribute);
        ...
        public string ToString(int lenth, Dictionary<string, object[]> dicIndexPropertes = null)
        {
            ...
            for (int i = 0; i < properties.Length; i++)
            {
                ...
                attrs = propertyInfo.GetCustomAttributes(NBaseModel._browsableType, false);
                if (attrs.Length != 0)
                {
                ...
                //显示名称
                attrs = propertyInfo.GetCustomAttributes(NBaseModel._displayNameType, false);
                if (attrs.Length == 0)
                {
                ...
                //生成行字符串
                attrs = propertyInfo.GetCustomAttributes(NBaseModel._formatAttributeType, false);
                if (attrs.Length != 0)
                {
                    try
                    {
                        value = string.Format(((NDisplayFormatAttribute)attrs[0]).FormatString, value);
                    }
                    catch (Exception ex)
                    {
                ...
                //计算行字符串中中文字符数
                lineChineseCharCount = NExtendString.CalculateChineseCharCount(line);
