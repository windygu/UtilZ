...
namespace UtilZ.Lib.Base.Threading
{
    ...
    public class BreezeParallelThread<T> : NThread
    {
        /// <summary>
        /// 参数队列集合
        /// </summary>
        ...
        /// 当前正在处理的平行线程集合
        /// </summary>
        private readonly List<NThread> _threads = new List<NThread>();
        ...
        internal BreezeParallelThread(IEnumerable<T> items, Action<CancellationToken, T> action, int count)
        {
        ...
        protected override void ExcuteThreadMethod(CancellationToken token)
        {
            ...
            while (true)
            {
                lock (this)
                {
                    ...
                    if (this._count != -1 && this._threads.Count == this._count)
                    {
                        break;
                    }
        ...
        private bool CreateTask(CancellationToken token)
        {
            ...
            NThread breeezeThread = NThread.Create<T>(new Action<CancellationToken, T>((subThreadToken, para) =>
                {
                    this.ProcessAction(token, para);
                }), this.IsBackground, this.Priority);

            breeezeThread.Exited += (sender, args) =>
            {
                lock (this)
                {
                    this._threads.Remove((NThread)sender);
                    this.CreateTask(token);
                }
            };

            this._threads.Add(breeezeThread);
            breeezeThread.Start(item);
            return true;
        }
