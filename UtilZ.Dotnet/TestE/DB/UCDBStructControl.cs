    ...
    public partial class UCDBStructControl : UserControl, ICollectionOwner
    {
        ...
        private void UCDBStructControl_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                List<DBConfigElement> items = ConfigManager.GetAllConfigItems();
                DropdownBoxHelper.BindingIEnumerableGenericToComboBox<DBConfigElement>(comboBoxDB, items, nameof(DBConfigElement.ConName));
                ...
                dgvTableFields.ShowData(_fieldInfoBindingList.DataSource, "UCDBStructControl.dgvTableFields");
                this.checkBoxAllTableField_CheckedChanged(sender, e);
            }
            catch (Exception ex)
            {
                Loger.Error(ex);
        ...
        private void btnLoad_Click(object sender, EventArgs e)
        {
            try
            {
                var dbConfig = DropdownBoxHelper.GetGenericFromComboBox<DBConfigElement>(comboBoxDB);
                var para = new PartAsynWaitPara<DBConfigElement, object>();
                ...
                para.Function = (p) =>
                {
                    ...
                    this._tableList = dbAccess.GetTableInfos(true).OrderBy(t => { return t.Name; }).Select(t => { return new DBTableInfoEx(t); }).ToList();
        ...
        private void checkBoxAllTableField_CheckedChanged(object sender, EventArgs e)
        {
        ...
        private string GetFiledClrTypeString(DBFieldInfo fieldInfo)
        {
            ...
            switch (dbFieldType)
            {
                ...
                case DBFieldType.Number:
                    if (fieldInfo.FiledName.EndsWith("ID", StringComparison.OrdinalIgnoreCase) ||
               fieldInfo.FiledName.EndsWith("no", StringComparison.OrdinalIgnoreCase))
                    {
                        typeStr = "long";
                    }
                    else
