...
namespace TestDB
{
    public partial class FTest : Form
    {
        ...
        private void FTest_Load(object sender, EventArgs e)
        {
            ...
            EFEntityTypeManager.RegisterEntityType(3, typeof(Stu));
        ...
        private bool CustomRegisteEntityTypeFunc(DatabaseConfig config, DbModelBuilder modelBuilder)
        {
            if (config.DBID == this._oracleDbid)
            {
                ...
                modelBuilder.Entity<StuOracle>().Property(p => p.ID).HasColumnName(nameof(StuOracle.ID)).HasDatabaseGeneratedOption(System.ComponentModel.DataAnnotations.Schema.DatabaseGeneratedOption.None);
                modelBuilder.Entity<StuOracle>().Property(p => p.Name).HasColumnName(nameof(StuOracle.Name));
                modelBuilder.Entity<StuOracle>().Property(p => p.Age).HasColumnName(nameof(StuOracle.Age));
                modelBuilder.Entity<StuOracle>().Property(p => p.Bir).HasColumnName(nameof(StuOracle.Bir));
            ...
            {
                //pstsql 全小写
                modelBuilder.Entity<Stu>().ToTable(nameof(Stu).ToLower(), string.Empty);
                modelBuilder.Entity<Stu>().Property(p => p.ID).HasColumnName(nameof(Stu.ID).ToLower());
                modelBuilder.Entity<Stu>().Property(p => p.Name).HasColumnName(nameof(Stu.Name).ToLower());
                modelBuilder.Entity<Stu>().Property(p => p.Age).HasColumnName(nameof(Stu.Age).ToLower());
                modelBuilder.Entity<Stu>().Property(p => p.Bir).HasColumnName(nameof(Stu.Bir).ToLower());
        ...
        private void SetValue(IStu stu, long id = -1)
        {
            if (id == -1)
            {
        ...
        private void QueryPageInfo(IDBAccess dbAccess)
        {
            try
            {
                ...
                dgv.SetPageInfo(new UtilZ.Dotnet.WindowEx.Winform.Controls.PageGrid.PageInfo(_pageSize, 1, pageInfo.TotalCount));
            }
            catch (Exception ex)
            {
                Loger.Error(ex);
        ...
        private void dgv_PageSizeChanged(object sender, UtilZ.Dotnet.WindowEx.Winform.Controls.PageGrid.PageSizeChangedArgs e)
        {
            _pageSize = e.PageSize;
            this.QueryPageInfo(this._dbAccess);
        }

        private void dgv_QueryData(object sender, UtilZ.Dotnet.WindowEx.Winform.Controls.PageGrid.QueryDataArgs e)
        {
            try
            {
        ...
        private void btnEF_Click(object sender, EventArgs e)
        {
            ...
            {
                var config = DropdownBoxHelper.GetGenericFromComboBox<DatabaseConfig>(comboBoxDB);
                using (var ef = new EFModelContext(config.DBID))
                {
                    ...
                    {
                        newRole.Action = new List<EFModel.Action>();
                    }
