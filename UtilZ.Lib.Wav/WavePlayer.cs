...
using UtilZ.Lib.Wav.Model;
using System.IO;
using System.Drawing.Drawing2D;
using System.Threading.Tasks;
using System.Diagnostics;

namespace UtilZ.Lib.Wav
{
    /// <summary>
    /// WAV播放控件
    /// </summary>
    ...
    public partial class WavePlayer : Control
    {
        ...
        private bool InitWavInfo(string fileName, out BASS_CHANNELINFO_INTERNAL channelInfo, out short[] srcData, out short[] leftData, out short[] rightData, out long totalLength, out double time, out bool isStereo)
        {
            ...
            try
            {
                ...
                short[] data = new short[totalLength];
                int count = Bass.BASS_ChannelGetData(handle, data, (uint)totalLength);
                //int count = Bass.BASS_ChannelGetData(handle, data, (uint)(BASS_ChannelGetDataLengthMode.BASS_DATA_FFT2048 | BASS_ChannelGetDataLengthMode.BASS_DATA_FFT_COMPLEX));
                //int count = Bass.BASS_ChannelGetData(handle, data, (uint)(BASS_ChannelGetDataLengthMode.BASS_DATA_FFT2048));
                if (count == -1)
                {
                    this.OnRaiseLog(BassErrorCode.GetErrorInfo());
                    srcData = null;
                ...
                //此处除以2,是因为不知道为何,通过Bass.BASS_ChannelGetData取回来的数据中,后面一半全是为0的无效数据,
                //正常的逻辑是上面注释掉的这部分代码
                ...
                if (isStereo)
                {
                    ...
                    {
                        channelDataLength = (uint)(count / 2);
                    }
                    else
                    {
