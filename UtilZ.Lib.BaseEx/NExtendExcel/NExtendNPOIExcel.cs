    ...
    public class NExtendNPOIExcel
    {
        ...
        public static void ExportToExcel(DataTable dt, string filePath, Dictionary<string, string> dicHeadCaptions = null)
        {
            ...
            ************************************************************************************/
            ...
            NExtendNPOIExcel.Export(workbook, dt, filePath, sheetMaxRow, dicHeadCaptions);
        }
            ...
            for (int i = 1; i <= sheetCount; i++)
            {
                ...
                for (int j = 1; j <= currentDataRowCount; j++)
                {
                    ...
                    for (int k = 0; k < colCount; k++)
                    {
                        ...
                        if (cellVelueType == NClrSystemType.BoolType)//bool类型
                        {
                            dataCell.SetCellValue(Convert.ToBoolean(cellValue));
                        }
                        else if (cellVelueType == NClrSystemType.DateTimeType)//时间类型
                        {
                            dataCell.SetCellValue(Convert.ToDateTime(cellValue));
                        }
                        else if (NClrSystemType.IsSystemNumberType(cellVelueType))//如果是系统数值类型
                        {
                            dataCell.SetCellValue(Convert.ToDouble(cellValue));
                        }
                        else if (cellVelueType == NClrSystemType.StringType || cellVelueType == NClrSystemType.CharType)//字符串或字符类型
                        {
