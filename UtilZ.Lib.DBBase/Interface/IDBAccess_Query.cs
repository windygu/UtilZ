...
namespace UtilZ.Lib.DBBase.Interface
{
    ...
    public partial interface IDBAccess
    {
        ...
        List<T> QueryT<T>(string orderByColName, int pageSize, int pageIndex, bool orderFlag, IEnumerable<string> queryColumns = null, IEnumerable<string> priKeyCols = null) where T : class, new();
        ...
        /// <param name="priKeyCols">主键集合[当为oracle时此值不能为空或null,否则查询出的结果可能出现重复,因为Oracle数据的分页查询中排序规则不稳定]</param>
        /// <returns>数据集合</returns>
        List<T> QueryT<T>(IEnumerable<DBOrderInfo> orderInfos, int pageSize, int pageIndex, bool orderFlag, IEnumerable<string> queryColumns = null, IEnumerable<string> priKeyCols = null) where T : class, new();
        #endregion
